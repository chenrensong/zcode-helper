...
bool ZcodeBridge::ClaimZcodeProtocol() {
  ...
  if (RegOpenKeyExW(HKEY_CURRENT_USER, kProtocolKey L"\\shell\\open\\command",
                    0, KEY_QUERY_VALUE, &existing) == ERROR_SUCCESS) {
...
void ZcodeBridge::RestoreZcodeProtocol() {
  if (had_protocol_command_) {
    ...
    if (RegCreateKeyExW(HKEY_CURRENT_USER,
                        kProtocolKey L"\\shell\\open\\command", 0, nullptr,
                        REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, nullptr,
                        &cmd_key, nullptr) == ERROR_SUCCESS) {
  ...
  std::thread([] {
    for (;;) {
      ...
      if (message.rfind("zcode://", 0) == 0) {
        ...
        MsgWindow().Post("\x01ACTIVATE");  // 借道主线程激活窗口
      }
    }
  }).detach();
}
